...
bool Configurations::Parser::parseFromFile(const std::string& configurationFile, Configurations* sender,
  ...
  ELPP_ASSERT(fileStream_.is_open(), "Unable to openFile configuration file [" << configurationFile << "] for parsing.");
...
void Loggers::configureFromGlobal(const char* globalConfigurationFilePath) {
  ...
  ELPP_ASSERT(gcfStream.is_open(), "Unable to openFile global configuration file [" << globalConfigurationFilePath
              << "] for parsing.");
  std::string line = std::string();
  std::stringstream ss;
